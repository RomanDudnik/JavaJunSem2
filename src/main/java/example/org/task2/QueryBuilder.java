...
public class QueryBuilder {
    ...
    /**
     * Построить запрос на обновление данных из бд
     *
     * @param obj
     * @return
     */
    ...
    public String buildDeleteQuery(Class<?> clazz, UUID primaryKey) throws IllegalAccessException {
        // Получаем имя таблицы из аннотации @Table
        Table table = clazz.getAnnotation(Table.class);
        String tableName = table.name();

        // Получаем имя столбца с первичным ключом из аннотации @Column
        String pkColumnName = null;
        for (Field field : clazz.getDeclaredFields()) {
            Column columnAnnotation = field.getAnnotation(Column.class);
            if (columnAnnotation != null && columnAnnotation.primaryKey()) {
                pkColumnName = columnAnnotation.name();
                break;
            }
        }

        // Формируем SQL-запрос для удаления
        return "DELETE FROM " + tableName + " WHERE " + pkColumnName + " = '" + primaryKey + "'";
    }
    */
